...
export interface VoltSectorConfig {
  width: number;
  height: number;
  text: string;
  fontFamily?: string;
...
export class VoltSectorEngine {
  private cfg: Required<VoltSectorConfig>;

  constructor(config: VoltSectorConfig) {
    // Merge provided configuration with static studio defaults
    const defaults: Required<VoltSectorConfig> = {
      width: 800,
      height: 200,
      text: "CYBER",
      fontFamily: "Anton",
  ...
  drawFrame(ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, ghostFrames?: ImageData[]): void {
    ...
    const isGlitchEffect = "VoltSector".toLowerCase().includes("glitch") || text === "SYSTEM ERR";
    if (isGlitchEffect) {
      ctx.save();
      ctx.globalAlpha = 0.8;
...
export const VoltSectorDefinition: TextEffectDefinition = {
  id: "volt-sector",
  name: "Volt Sector",
  text: "CLYPRA",
  category: "classic",
  description: "A custom Canvas 2D text effect named Volt Sector with solid fill.",
